...
df.pre.reads<-tail(df.pre.reads,50)
df.post.reads<-head(df.post.reads,50)

df.post.reads$initpos<-df.post.reads$initpos/1000000
df.pre.reads$initpos<-df.pre.reads$initpos/1000000

if (is(cov_normalization)[1]=="character"){
...
plot.new()
pdf(paste0(output.file,"_zoomin20k.coverageplot.pdf"))
#plot.new()
